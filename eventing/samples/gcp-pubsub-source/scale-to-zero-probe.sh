...
do
    ...
    gcloud pubsub topics publish testing --message="$WANT" 2>&1
    sleep 30
    kubectl get deploy 2>&1 | when

    echo "Looking for the random message" | when
    GOT=$(kubectl logs -l serving.knative.dev/service=message-dumper -c user-container | grep '{' | jq1.6 -r '.Data | @base64d' | grep "$WANT" 2>&1)
